...
function createAlgodClient() {
  try {
    ...
    console.log(`[Algorand Client] Initializing Algod client with URL: ${server}, Port: ${port}, Token: ${token ? '***' : 'None'}`);
    // Create and return the algod client
    return new algosdk.Algodv2(token, server, port);
  } catch (error) {
    console.error('[Algorand Client] Failed to create Algod client:', error);
    return null;
  }
}
...
function createIndexerClient() {
  try {
    ...
    console.log(`[Algorand Client] Initializing Indexer client with URL: ${server}, Port: ${port}, Token: ${token ? '***' : 'None'}`);
    // Create and return the indexer client
    return new algosdk.Indexer(token, server, port);
  } catch (error) {
    console.error('[Algorand Client] Failed to create Indexer client:', error);
    return null;
  }
}
...
if (!validation.valid) {
  ...
  console.error('[Algorand Client] Please check your .env.local file and ensure all required variables are set.');
} else {
  console.log('[Algorand Client] All required environment variables are present.');
}
...
if (algodClient) {
  console.log('[Algorand Client] Algod client initialized successfully.');
} else {
  console.error('[Algorand Client] Algod client failed to initialize.');
}
if (indexerClient) {
  console.log('[Algorand Client] Indexer client initialized successfully.');
} else {
  console.error('[Algorand Client] Indexer client failed to initialize.');
}
...
export { algodClient, indexerClient, isInitialized, getNetworkType, getExplorerURL };